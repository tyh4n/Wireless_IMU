...
/*!
 *  @brief  apply SIC calibration matrix to BNO055
 */
void Adafruit_BNO055::applySICmatrix(float factor0, float factor1, float factor2, float factor3, float factor4, float factor5,float factor6,float factor7,float factor8){
  adafruit_bno055_opmode_t lastMode = _mode;
  setMode(OPERATION_MODE_CONFIG);
  delay(25);
  applySICmatrixFactor(BNO055_SIC_MATRIX_0_LSB_ADDR,BNO055_SIC_MATRIX_0_MSB_ADDR,factor0);
  applySICmatrixFactor(BNO055_SIC_MATRIX_1_LSB_ADDR,BNO055_SIC_MATRIX_1_MSB_ADDR,factor1);
  applySICmatrixFactor(BNO055_SIC_MATRIX_2_LSB_ADDR,BNO055_SIC_MATRIX_2_MSB_ADDR,factor2);
  applySICmatrixFactor(BNO055_SIC_MATRIX_3_LSB_ADDR,BNO055_SIC_MATRIX_3_MSB_ADDR,factor3);
  applySICmatrixFactor(BNO055_SIC_MATRIX_4_LSB_ADDR,BNO055_SIC_MATRIX_4_MSB_ADDR,factor4);
  applySICmatrixFactor(BNO055_SIC_MATRIX_5_LSB_ADDR,BNO055_SIC_MATRIX_5_MSB_ADDR,factor5);
  applySICmatrixFactor(BNO055_SIC_MATRIX_6_LSB_ADDR,BNO055_SIC_MATRIX_6_MSB_ADDR,factor6);
  applySICmatrixFactor(BNO055_SIC_MATRIX_7_LSB_ADDR,BNO055_SIC_MATRIX_7_MSB_ADDR,factor7);
  applySICmatrixFactor(BNO055_SIC_MATRIX_8_LSB_ADDR,BNO055_SIC_MATRIX_8_MSB_ADDR,factor8);
  setMode(lastMode);
}

/*!
 *  @brief  apply single SIC calibration value to BNO055
 */
void Adafruit_BNO055::applySICmatrixFactor(adafruit_bno055_reg_t LSB_reg, adafruit_bno055_reg_t MSB_reg, float value){
  int16_t valueInt = value * 16384;

  byte LSBData = valueInt & 0xff;
  byte MSBData = valueInt>>8;
  
  write8(LSB_reg,LSBData);
  write8(MSB_reg,MSBData);
}
