...
int minConnectedDevice = 1;  //less than this number of device connected, the imu station goes to sleep
int stationMode = 1;

//==================BNO055 config==================
/* Set the delay between fresh samples */
//#define BNO055_SAMPLERATE_DELAY_MS 8 //delay-based frequency control
#define BNO055_PERIOD 30  //ms, 100Hz
// Check I2C device address and correct line below (by default address is 0x29 or 0x28)
//                                   id, address
Adafruit_BNO055 bno = Adafruit_BNO055(-1, 0x28, &Wire);

//Delaying BNO055 data with queue
cppQueue localIMUXQueue(4,10,FIFO);
cppQueue remoteIMUXQueue(4,4,FIFO);
// ================== BLE config==================
...
//BLE Client Service
#define IMUX_ENABLE 1
#define IMUY_ENABLE 0
#define IMUZ_ENABLE 0
#define IMUALL_ENABLE 0
BLEClientService client_imus = BLEClientService(IMU_SERVICE_UUID);
BLEClientCharacteristic client_imux = BLEClientCharacteristic(IMU_EULAR_X_UUID);
float clientIMUX = 0;
uint16_t connectedWirelessIMU = 0;
//===================SLEEP CONFIG================
#define WAKE_LOW_PIN PIN_A0
#define WAKE_HIGH_PIN PIN_A1
#define SLEEPING_DELAY 100  //unit of BNO055_PERIOD
...
void setup(void) {
  //=================setup usb serial//==================
  if (DebugMode || stationMode) Serial.begin(115200);
  ...
  Bluefruit.begin(2, 1);
  //set device name
  Bluefruit.setName("PURE Wireless IMU station");
  ...
  //==================setup BLE as central//==================
  if (stationMode) {
    setupIMUClient();
    startScan();
  }

  suspendLoop();  //dont use loop here due to instability when suspending and resuming
}
...
void updateData(void* arg) {  //main loop
  ...
  while (1) {
    ...
    } else {  //sleep after a while if the numbre of connected device is less than a thershold
      sleep_timeout--;
...
void postIMUEular() {
  ...
  if (stationMode) {
    //dealing with zero crossing
    float angleDifference = clientIMUX - eularSend[0];
    ...
    Serial.print(" \t ");
    Serial.print(clientIMUX);
    Serial.print(" \t ");
    Serial.println(eularSend[0]);
  }
}
void CalaulateAngleDifference(){
...
void applySensorOffsets() {
  ...
  calibData.accel_offset_x = 88;
  calibData.accel_offset_y = -15;
  calibData.accel_offset_z = 19;

  calibData.gyro_offset_x = 0;
  calibData.gyro_offset_y = 0;
  calibData.gyro_offset_z = 1;

  calibData.mag_offset_x = -68;
  calibData.mag_offset_y = -164;
  calibData.mag_offset_z = -264;

  calibData.accel_radius = 1000;

  calibData.mag_radius = 744;
  bno.setSensorOffsets(calibData);
}

// for connecting to another IMU
void setupIMUClient() {
  client_imus.begin();
  client_imux.setNotifyCallback(client_imux_notify_callback);
  client_imux.begin();
}
void connectIMUclient(uint16_t conn_handle) {
  // If wireless imu service is not found on the connected target, disconnect and return
  if (!client_imus.discover(conn_handle)) {
    Serial.println("connected but not wireless imu");
    // disconnect since we couldn't find HRM service
    Bluefruit.disconnect(conn_handle);
    return;
  }
  // Once HRM service is found, we continue to discover its characteristic
  Serial.println("wireless imu connected");
  connectedWirelessIMU = conn_handle;
  if (IMUX_ENABLE) {
    Serial.print("Discovering IMUX");
    if (!client_imux.discover()) {
      // Measurement chr is mandatory, if it is not found (valid), then disconnect
      Serial.println("imux characteristic is not found");
      Bluefruit.disconnect(conn_handle);
      return;
    }
    Serial.println("imux Found");
    if (client_imux.enableNotify()) {
      Serial.println("imux enabled");
      wakeup();
    } else {
      Serial.println("cannot enable imux");
    }
  }
}

void central_scan_callback(ble_gap_evt_adv_report_t* report) {
  //connect once the wireless imu is found
...
void central_connect_callback(uint16_t conn_handle) {
  minConnectedDevice = 1;

  Serial.println("Connected TO WIRELESS IMU");
  connectIMUclient(conn_handle);
}
...
int sendingIntervalNotify = 0;
int lastSendingNotify = 0;
void client_imux_notify_callback(BLEClientCharacteristic* chr, uint8_t* data, uint16_t len) {
  sendingIntervalNotify = millis() - lastSendingNotify;
  lastSendingNotify = millis();
  // Serial.print("remote IMU Interval: ");
  // Serial.println(sendingIntervalNotify);
  float* pointer = (float*)data;
  float floatdata = *pointer;
  clientIMUX = floatdata;
...
void startScan(void) {
  ...
  Bluefruit.Scanner.setRxCallback(central_scan_callback);
  Bluefruit.Scanner.restartOnDisconnect(true);
  Bluefruit.Scanner.setInterval(160, 80);          // in unit of 0.625 ms
  Bluefruit.Scanner.filterUuid(client_imus.uuid);  //find PURE wireless imu with its uuid (not mac address so any pure wireless imu would work)
...
void goSleep() {
  if (!sleeping) {
    ...
    Bluefruit.disconnect(connectedWirelessIMU);  //DISCONNECT THE WIRELESS IMU
    if (DebugMode) postDebugMessage(String("sleeping, waiting for connection"));
